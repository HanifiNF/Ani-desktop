...
- **macOS:** the existing ad-hoc-signed DMGs use manual replacement. Electron's Squirrel.Mac updater requires a suitable signing identity; release-to-release ad-hoc signatures do not provide a stable Developer ID identity. To enable that path, obtain a Developer ID Application certificate, configure signing and notarization in CI, generate both DMG and ZIP targets for each architecture, and publish the matching macOS update metadata. Running from a mounted DMG or a translocated/read-only app location also prevents reliable replacement; install in Applications first. Apple notarization provides the standard downloaded-app Open prompt. Sparkle supports a free alternative using project-owned Ed25519 update signatures and ad-hoc bundle signing. A local fixture update and invalid-signature rejection have been verified; the Electron integration remains pending. See the [Sparkle investigation](docs/sparkle-investigation.md). Apple notarization remains a separate Gatekeeper concern.

Apple credentials come from the [Apple Developer Program](https://developer.apple.com/programs/enroll/). Create a [Developer ID Application certificate](https://developer.apple.com/help/account/certificates/create-developer-id-certificates) and export it with its private key as a password-protected `.p12`. For CI, store the certificate as `CSC_LINK` (base64) and its password as `CSC_KEY_PASSWORD` in GitHub Actions secrets. Notarization can use `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, and `APPLE_TEAM_ID`, or an App Store Connect API key. The current build deliberately retains ad-hoc signing until this configuration is supplied; adding secrets alone does not change the macOS update mode.
