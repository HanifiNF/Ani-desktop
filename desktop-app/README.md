...
- Browse uses AniList to show 24 catalog titles at a time with genre, year, season, status, format, score, episode-count, and sorting filters. Filter edits only request data when applied, pages are cached, and streaming providers are searched only after a title is opened. A catalog title with no matching provider remains visible as information without being added to history.
