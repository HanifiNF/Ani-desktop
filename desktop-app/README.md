...
- Home shows the current seasonal schedule below Saved as a poster-card grid. It opens on today in a rolling seven-day strip (two days back, today, four days ahead), which shifts at local midnight. Each day is requested from AniWave by date, which also covers days the site's own schedule tabs no longer list. It supports SUB/DUB, orders AniWave's estimated releases by local time, shows a countdown and one line of genre chips per card (with a "+n" chip for any that do not fit), and dims entries that have not aired yet while leaving aired ones at full brightness. AniWave must be enabled for live schedule updates; cached rows remain visible with a stale notice during an outage.
