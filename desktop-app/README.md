...
- Home shows the current seasonal schedule below Saved as a poster-card grid. It opens on today in the local Sunday–Saturday week, supports SUB/DUB, orders AniWave's estimated releases by local time, shows a countdown and up to two genre chips per card, and dims entries that have not aired yet while leaving aired ones at full brightness. AniWave must be enabled for live schedule updates; cached rows remain visible with a stale notice during an outage.
