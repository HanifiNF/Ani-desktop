...
- Series pages show information for the work: format and season, airing status with the next episode date, studio, score, a synopsis, genres, and related seasons that open as a new search. It comes from AniList, looked up by the work's ids or, for a work without ids yet, by its titles, and is cached locally: a finished series is fetched once and refreshed after about ninety days, an airing one after a day, or on **Refresh info**. Episode counts and lists always come from the streaming sources. The AniList switch in Settings turns these requests off; cached information and provider-scraped genres remain. An optional offline title index (a local copy of the anime-offline-database, about six megabytes, refreshed weekly) lets search group titles across sources without any request.
